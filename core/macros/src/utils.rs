...
pub type SpannedResult<T> = Result<T, (Span2, String)>;

/// A function to make it easier to return error messages.
pub fn error<T>(span: &impl Spanned, message: impl Display) -> SpannedResult<T> {
    Err((span.span(), message.to_string()))
}

/// Look (and remove from AST) a `path` version of the attribute `boa`, e.g. `#[boa(something)]`.
pub fn take_path_attr(attrs: &mut Vec<Attribute>, name: &str) -> bool {
...
pub fn take_name_value_attr(attrs: &mut Vec<Attribute>, name: &str) -> Option<Lit> {
...
pub fn take_length_from_attrs(attrs: &mut Vec<Attribute>) -> SpannedResult<Option<usize>> {
...
pub fn take_error_from_attrs(attrs: &mut Vec<Attribute>) -> SpannedResult<Option<String>> {
...
impl RenameScheme {
    pub fn from_attrs(attrs: &mut Vec<Attribute>) -> SpannedResult<Self> {
        match take_name_value_attr(attrs, "rename") {
            None => Ok(Self::default()),
            Some(Lit::Str(lit_str)) => {
                Self::from_str(lit_str.value().as_str()).map_err(|e| (lit_str.span(), e))
            }
            Some(lit) => Err((
                lit.span(),
